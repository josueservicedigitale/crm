...
class CorbeilleController extends Controller
{
    ...
    public function index(Request $requete)
    {
        ...
        $statistiques = [
            ...
            'utilisateurs' => Corbeille::where('type_element', User::class)->count(),
            'societes' => Corbeille::where('type_element', Societe::class)->count(),
            'activites' => Corbeille::where('type_element', Activite::class)->count(),
            'documents' => Corbeille::where('type_element', Document::class)->count(),

            // ✅ AJOUT
            'dossiers' => Corbeille::where('type_element', Dossier::class)->count(),
            'fichiers' => Corbeille::where('type_element', Fichier::class)->count(),
        ];
    ...
    public function restaurer($id)
    {
        ...
        try {
            ...
            Log::info("♻️ Restauration depuis corbeille", [
                'type' => $element->type_element,
                'nom_type' => $element->nom_type,
                'element_id' => $element->element_id,
                'restaure_par' => Auth::id(),
            ]);
        ...
        try {
            ...
            if (class_exists($classeModele)) {
                ...
                if ($modele) {
                    ...
                    if ($classeModele === Fichier::class) {
                        if ($modele->chemin && \Storage::disk('public')->exists($modele->chemin)) {
                            \Storage::disk('public')->delete($modele->chemin);
                        }
                    }
                    $modele->forceDelete();
